...
func init() {
	...
	if err != nil {
		utils.WriteGatewayAudit(fmt.Sprintf("unabled to determine default timeout value %v", err.Error()), "CONFIG", "CONFIG")
		timeoutSeconds = 10
	}
...
func (p *Pull) GetTeamDatasetsFMA(teamId int) (pkg.DatasetsVersions, error) {
	...
	if err != nil {
		...
		if string(body) == "[]" {
			...
			utils.WriteGatewayAudit("No existing FMA datasets found for this team", customAction, "GET")
...
func isTimeToRun(fed *pkg.Federation) bool {
	...
	if dt.Hour() != fed.RunTimeHour {
		...
		utils.WriteGatewayAudit(fmt.Sprintf(customMsg, fed.ID, fed.RunTimeHour, dt.Hour()), customAction, "GET")
		return false
	}
